...
public class Theory3_4Manager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Привязываем методы к кнопкам
         openButton.onClick.AddListener(() => LoadScene("Theory3-3"));
        actionButton.onClick.AddListener(() => LoadScene("Home"));  // Название сцены, на которую мы переходим
        nextSceneButton.onClick.AddListener(() => LoadScene("Home"));  // Переход на сцену theory2-2
