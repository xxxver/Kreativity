...
public class Theory3_3Manager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Привязываем методы к кнопкам
         openButton.onClick.AddListener(() => LoadScene("Theory3-2"));
        actionButton.onClick.AddListener(() => LoadScene("Home"));  // Название сцены, на которую мы переходим
        nextSceneButton.onClick.AddListener(() => LoadScene("theory3-4"));  // Переход на сцену theory2-2
